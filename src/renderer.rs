...
impl NativeRenderer {
  ...
  #[expect(
    ...
    reason = "PyO3 exposes the existing Python keyword-argument API as a Rust method signature."
  )]
  pub fn render_compiled(
    ...
    let encoder_settings = ImageEncoderSettings {
    ...
    let output =
      py.detach(move || render_to_vec(resources, node, stylesheet, settings, encoder_settings))?;
  ...
  pub fn measure_compiled(
    ...
    let measured = py.detach(move || measure_to_node(&resources, node, stylesheet, settings))?;
  ...
  pub fn render_svg_compiled(
    ...
    py.detach(move || render_svg_to_string(&resources, node, stylesheet, settings))
  }
...
impl NativeRenderer {
  ...
  fn write_legacy_images(&self) -> PyResult<RwLockWriteGuard<'_, HashMap<Arc<str>, ImageSource>>> {
...
fn normalize_node_input_for_serde(py: Python<'_>, value: Bound<'_, PyAny>) -> PyResult<Py<PyAny>> {
  if let Ok(bytes) = value.cast::<PyBytes>() {
    return Ok(
      PyList::new(py, bytes.as_bytes().iter().copied())?
        .into_any()
        .unbind(),
    );
  }
...
fn render_to_vec(
  ...
  encoder_settings: ImageEncoderSettings,
) -> PyResult<Vec<u8>> {
  let image = render_to_bitmap(&resources, node, stylesheet, settings)?;
  encode_static_image(image, encoder_settings)
}
...
fn render_to_bitmap(
  ...
  let fonts = read_lock(&resources.fonts, "renderer font lock poisoned")?;
...
fn encode_static_image(image: Bitmap, settings: ImageEncoderSettings) -> PyResult<Vec<u8>> {
  let Some(image_format) = settings
    .format
    .image_format(settings.quality, settings.lossless)?
  else {
